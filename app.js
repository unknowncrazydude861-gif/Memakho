...
// Dark faceted fragments inspired by an editorial, archaeological atmosphere.
const shardGroup=new THREE.Group();world.add(shardGroup);
const shardMats=[
 new THREE.MeshStandardMaterial({color:0x11130f,roughness:1,metalness:0}),
 new THREE.MeshStandardMaterial({color:0x1a1b16,roughness:1,metalness:.03}),
 new THREE.MeshStandardMaterial({color:0x2a2a23,roughness:.95,metalness:.02})
];
for(let i=0;i<28;i++){
 const geo=new THREE.IcosahedronGeometry(.08+Math.random()*.42,0);
 const s=new THREE.Mesh(geo,shardMats[i%shardMats.length]);
 const a=Math.random()*Math.PI*2;
 const rad=3.4+Math.random()*5.8;
 s.position.set(Math.cos(a)*rad,(Math.random()-.5)*5.5,Math.sin(a)*rad*.38-1.8);
 s.rotation.set(Math.random()*2,Math.random()*2,Math.random()*2);
 s.userData={a,rad,phase:Math.random()*Math.PI*2,speed:.15+Math.random()*.45};
 shardGroup.add(s);
}

const goldGroup=new THREE.Group();world.add(goldGroup);
for(let i=0;i<11;i++){
 const geo=Math.random()>.55?new THREE.OctahedronGeometry(.08+Math.random()*.18,0):new THREE.TetrahedronGeometry(.09+Math.random()*.15,0);
 const m=new THREE.MeshBasicMaterial({color:0xdccf8f,transparent:true,opacity:.58+Math.random()*.3});
 const s=new THREE.Mesh(geo,m);
 const a=Math.random()*Math.PI*2,rad=3.2+Math.random()*5.2;
 s.position.set(Math.cos(a)*rad,(Math.random()-.5)*5.2,Math.sin(a)*rad*.42-1.3);
 s.userData={a,rad,phase:Math.random()*6,speed:.2+Math.random()*.5};
 goldGroup.add(s)
}

const particleGroup=new THREE.Group();world.add(particleGroup);
for(let i=0;i<75;i++){
 const p=new THREE.Mesh(new THREE.SphereGeometry(Math.random()*.035+.012,8,8),new THREE.MeshBasicMaterial({color:i%3===0?0xd6ca9f:0x9aa98b,transparent:true,opacity:.22+Math.random()*.42}));
 const a=Math.random()*Math.PI*2,rad=3+Math.random()*6;
 p.position.set(Math.cos(a)*rad,(Math.random()-.5)*7,Math.sin(a)*rad*.45-2);
 p.userData={a,r:rad,s:.002+Math.random()*.006,phase:Math.random()*6};particleGroup.add(p)
}
...
function makeCard(tex,index,total){
 const aspect=1.16;
 const geo=new THREE.PlaneGeometry(2.45,2.45/aspect);
 const mat=new THREE.MeshPhysicalMaterial({map:tex,color:0xffffff,roughness:.6,metalness:.08,transparent:true,side:THREE.DoubleSide});
 const mesh=new THREE.Mesh(geo,mat);
 mesh.userData.base=index;mesh.userData.total=total;mesh.userData.seed=Math.random()*10;
 const frame=new THREE.LineSegments(new THREE.EdgesGeometry(geo),new THREE.LineBasicMaterial({color:0xe3d6b3,transparent:true,opacity:.5}));
 mesh.add(frame);photoGroup.add(mesh);photoMeshes.push(mesh);return mesh
}
async function rebuildPhotoMeshes(){
 for(const m of photoMeshes){m.material.map?.dispose();m.material.dispose();m.geometry.dispose();photoGroup.remove(m)}
 photoMeshes.length=0;
 const list=state.photos.length?state.photos.map(p=>p.url):demoPhotos;
 for(let i=0;i<list.length;i++){const tex=await makePhotoTexture(list[i]);if(tex)makeCard(tex,i,list.length)}
 layoutPhotos()
}
function layoutPhotos(){
 const total=photoMeshes.length||1;
 photoMeshes.forEach((m,i)=>{
  const a=(i/Math.max(total,1))*Math.PI*2;
  const radius=4.05+(i%3)*.43;
  m.userData.a=a;m.userData.radius=radius;
  m.position.set(Math.cos(a)*radius,Math.sin(a*1.35)*1.55,Math.sin(a)*1.3-1.1);
  m.rotation.set(-.08*Math.sin(a),-a+.35,.08*Math.cos(a));
  m.scale.setScalar(.92+(i%2)*.08)
 })
}

function updateWorld(){
 state.sceneProgress+=(state.targetProgress-state.sceneProgress)*.055;
 const p=state.sceneProgress,px=state.pointerX,py=state.pointerY;
 const desiredX=3.35-Math.min(p,1.25)*2.45;
 const desiredY=.15-p*.22;
 world.position.x+=(desiredX+px*.45-world.position.x)*.04;
 world.position.y+=(desiredY-py*.22-world.position.y)*.04;
 world.position.z+=(-.25+Math.sin(p*Math.PI)*.5-world.position.z)*.04;
 world.rotation.y+=(px*.16+Math.sin(p*1.8)*.045-world.rotation.y)*.04;
 world.rotation.x+=(-py*.08+Math.cos(p*1.35)*.02-world.rotation.x)*.04;
 orb.rotation.y+=.0018;orb.rotation.x+=.00025;inner.rotation.y-=.00125;
 photoGroup.rotation.y+=.00035;
 shardGroup.rotation.y+=.00055;
 goldGroup.rotation.y-=.0006;
 particleGroup.rotation.y+=.001;
 const now=performance.now()*.001;
 shardGroup.children.forEach((s,i)=>{const u=s.userData; s.position.y+=Math.sin(now*u.speed+u.phase)*.00075; s.rotation.x+=.0007; s.rotation.z-=.0005;});
 goldGroup.children.forEach((s,i)=>{const u=s.userData;s.rotation.x+=.002;s.rotation.y+=.003;s.position.y+=Math.sin(now*u.speed+u.phase)*.001});
 photoMeshes.forEach((m,i)=>{
  const a=m.userData.a+now*.028*(i%2?1:-1),r=m.userData.radius;
  m.position.x=Math.cos(a)*r;
  m.position.y=Math.sin(a*1.35)*1.55;
  m.position.z=Math.sin(a)*1.3-1.1+(p>1.35?Math.sin(i)*.3:0);
  m.rotation.y=-a+.35+px*.08;
  m.rotation.x=-.08*Math.sin(a)+py*.05;
  const pulse=1+.035*Math.sin(now*1.3+i);
  m.scale.setScalar((.94+(i%2)*.06)*pulse)
 });
 key.position.x=5+px*2;key.position.y=4-py*2;
...
$('#importBtn').onclick=()=>$('#photoInput').click();
$('#folderBtn').onclick=()=>$('#folderInput').click();
$('#importTop').onclick=()=>$('#photoInput').click();
$('#photoTool').onclick=()=>$('#photoInput').click();
